--[[todo:
preview color
gui for making custom strobes?
rewrite the whole fukken thin


changed:
Rewrote the whole thing for the... fourth time? idk dude i just work here
Removed global function "lp_log" and replaced it with LasersPlus:log()

--]]
_G.LasersPlus = _G.LasersPlus or {}
LasersPlus._path = ModPath
LasersPlus.savename_1 = "lasersplus.txt" --these are saved separately so i can check for filenames
LasersPlus.savename_2 = "lasersplus_strobe.txt"
LasersPlus.savename_3 = "lasersplus_test.txt" --temp file; creating this in your saves folder will generate a strobe save.
LasersPlus._data_path = SavePath .. LasersPlus.savename_1
LasersPlus._strobe_save_path = SavePath .. LasersPlus.savename_2

--can be any turret theme_type

LasersPlus.SavedTeamStrobes = LasersPlus.SavedTeamStrobes or {}
LasersPlus.SavedTeamColors = LasersPlus.SavedTeamColors or {}
LasersPlus.SavedTeamFlashlights = LasersPlus.SavedTeamFlashlights or {}
--*****    Internal Values    *****
--Things that should not be changed
LasersPlus.LuaNetID = "lasersplus"
--these aren't used
...
--*****    log util function w/prepended string identifier    *****
function LasersPlus:log(message,name,tochat)
--todo if console
	if LasersPlus.debug_logs then
		if tochat then 
...
function LasersPlus:GetLaserQuality()
	return self.settings.quality or 5
end

function LasersPlus:FilterRedLasers(col) --todo add this check to all team playerlasers with toggle in options
...
--returns true if does not contain too much red, or if filter option is off
	if not col then
		self:log("Bad argument color to filter function")
		return false --sanity checker? i 'ardly even know 'er
	end
	if not self.settings.enabled_redfilter then
		self:log("Red Filter is not enabled")
		return col
	end
...
function Lasers:StrobeTableToString(data)
	if not data or type(data) ~= "table" or not data.colors then
		LasersPlus:log("Invalid table data!")
		return
	end 
...
function Lasers:StringToStrobeTable(data)
	...
	for k,v in ipairs(split_strobe) do
		if not v then
			LasersPlus:log("ERROR: Found invalid character in split string! Skipping...")
		elseif k == 1 then
--			LasersPlus:log("Skipping first key.")
		elseif v then
			output.color_count = k-1
			output.colors[output.color_count] = LuaNetworking:StringToColour(v)
		else
			LasersPlus:log("ERROR: Didn't find another color to add to the new strobe table...")
		end
	end
...
function Lasers:SaveStrobes(override)
	...
	if not file_exists then
		LasersPlus:log("No strobe save file found at" .. Lasers._strobe_save_path .. ".")
		return 
	end
...
function LasersPlus:log(message)
